...
class EffAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., act_layer=nn.ReLU):
        super().__init__()
...
class Conv(nn.Module):
    def __init__(self, nIn, nOut, kSize, stride, padding, dilation=(1, 1), groups=1, bn_acti=False, bias=False,
        ...
        if self.bn_acti:
            self.bn_relu = BN_ReLU(nOut, act_layer=act_layer)
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, nin, nout, kernel_size=3, stride=1, padding=1, dilation=1, bias=False, act_layer=nn.ReLU):
        super().__init__()
...
class DynamicConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, act_layer=nn.ReLU):
...
class self_net(nn.Module):
    def __init__(self, classes=4):
        ...
        self.init_conv = nn.Sequential(
            DepthwiseSeparableConv(3, 32, 3, 1, padding=1, act_layer=nn.ReLU),
            DepthwiseSeparableConv(32, 32, 3, 1, padding=1, act_layer=nn.ReLU),
            DepthwiseSeparableConv(32, 32, 3, 2, padding=1, act_layer=nn.ReLU),
        )

        self.bn_relu_1 = BN_ReLU(32)
        self.bn_relu_2 = BN_ReLU(64)
        self.bn_relu_3 = BN_ReLU(128)
        self.bn_relu_4 = BN_ReLU(32)
        self.bn_relu_5 = BN_ReLU(16)
        self.bn_relu_6 = BN_ReLU(16)
        self.bn_relu_7 = BN_ReLU(16)
        ...
        for i in range(3):
            self.Block_1.add_module(f"OCBlock_1_{i}", OCBlock(64, 64, use_attention=True))

        self.Block_2 = nn.Sequential()
        for i in range(12):
            self.Block_2.add_module(f"OCBlock_2_{i}", OCBlock(128, 128, use_attention=True))

        self.Block_3 = nn.Sequential()
        for i in range(12):
            self.Block_3.add_module(f"OCBlock_3_{i}", OCBlock(32, 32, use_attention=True))

        self.transformer1 = TransBlock(dim=288)

        self.Block_4 = nn.Sequential()
        for i in range(3):
            self.Block_4.add_module(f"OCBlock_4_{i}", OCBlock(32, 32, use_attention=True))
        self.upsample_1 = UpsampleingBlock(32, 16)
        self.Block_5 = nn.Sequential()
        for i in range(3):
            self.Block_5.add_module(f"OCBlock_5_{i}", OCBlock(16, 16, use_attention=True))
        self.upsample_2 = UpsampleingBlock(16, 16)
        self.Block_6 = nn.Sequential()
        for i in range(3):
            self.Block_6.add_module(f"OCBlock_6_{i}", OCBlock(16, 16, use_attention=True))
