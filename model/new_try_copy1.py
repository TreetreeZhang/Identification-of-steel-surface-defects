...
class CondConv2D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, num_experts=4, bias=False):
        super().__init__()
        ...
        if bias:
            self.biases = nn.Parameter(torch.randn(num_experts, out_channels))

    def forward(self, x):
        batch_size, in_channels, height, width = x.size()
        # 条件生成器：根据输入计算每个专家的权重
        condition = F.adaptive_avg_pool2d(x, 1).view(batch_size, in_channels)
        expert_weights = torch.sigmoid(self.fc(condition))  # 大小为 [batch_size, num_experts]

        # 初始化卷积核
        kernel = torch.sum(self.weight * expert_weights.view(batch_size, self.num_experts, 1, 1, 1, 1), dim=1)

        # 执行条件卷积
        out = F.conv2d(x, kernel.view(-1, in_channels, self.kernel_size, self.kernel_size), stride=self.stride, padding=self.padding, dilation=self.dilation, groups=batch_size)

        if self.bias:
            bias = torch.sum(self.biases * expert_weights, dim=1)
            out += bias.view(batch_size, -1, 1, 1)
        return out
...
class self_net(nn.Module):
    def __init__(self, classes=4, block_1=3, block_2=12, block_3=12, block_4=3, block_5=3, block_6=3):
        ...
        for i in range(0, block_1):
            self.DAB_Block_1.add_module("DAB_Module_1_" + str(i), DABModuleOptimized(64, d=2))
        ...
        for i in range(0, block_2):
            self.DAB_Block_2.add_module("DAB_Module_2_" + str(i), DABModuleOptimized(128, d=dilation_block_2[i]))
        ...
        for i in range(0, block_3):
            self.DAB_Block_3.add_module("DAB_Module_3_" + str(i), DABModuleOptimized(32, d=dilation_block_3[i]))
        ...
        for i in range(0, block_4):
            self.DAB_Block_4.add_module("DAB_Module_4_" + str(i), DABModuleOptimized(32, d=dilation_block_4[i]))
        ...
        for i in range(0, block_5):
            self.DAB_Block_5.add_module("DAB_Module_5_" + str(i), DABModuleOptimized(16, d=dilation_block_5[i]))
        ...
        for i in range(0, block_6):
            self.DAB_Block_6.add_module("DAB_Module_6_" + str(i), DABModuleOptimized(16, d=dilation_block_6[i]))
